"""v4 — Catimind-inspired pipeline: setting→design→storyboard"""
import json
...
# ── setting: 角色·场景文字设定 ──
cards.append(c("g-set-r3", "setting", "R3 苍霖", 1,
    ...
    f"## 可变层（表情）\n"
    f"隐忍：牙关咬紧眼眶泛红 | 空洞：瞳孔微放大嘴唇微张 | 微笑：极珍贵EP04\n\n"
    f"## 固定词\n{CANG}",
    episode=None, meta={"type": "character", "r_id": "R3"}))

cards.append(c("g-set-r1", "setting", "R1 艾拉", 2,
    f"## 身份层\n成年年轻女性 | 修长 | 黑长发银蓝高光 | 蓝瞳 | 偏白肤\n\n"
    f"## 装备层（系统固定词）\n{AILA}\n\n"
    f"## 魔法特效\n{AILA_VFX} 干净层次分明\n\n"
    f"## 可变层\n冷淡精确，不解释不安慰 | 出场：EP03",
    episode=None, meta={"type": "character", "r_id": "R1"}))

cards.append(c("g-set-r2", "setting", "R2 巨龙", 3,
    f"## 外形固定词\n{DRAGON}\n\n"
    f"## 成长阶段\n幼龙EP04：巴掌大鳞片稀疏 | 少龙EP05-07：猫大 | 成龙EP09-10：马大可骑乘\n\n"
    f"## 性格\n外凶内萌 | 出场EP04",
    episode=None, meta={"type": "character", "r_id": "R2"}))

cards.append(c("g-set-miner", "setting", "老矿工（路人）", 4,
    "EP08出场，劝阻苍霖别离开\n路人角色，无固定词，中年矿工形象",
    episode=None, meta={"type": "character"}))

cards.append(c("g-set-sc01", "setting", "scene01 烬龙陆废巷", 5,
    "## 空间结构\n窄巷道碎石墙壁，远处龙骨化石如山，地面粗糙散落碎片和废矿轨\n\n"
    "## 光影设计\n主光：暗红天光穿缝隙体积光斑\n补光：微弱蓝色光粒子漂浮\n色温：暖红主+冷蓝点缀\n\n"
    "## 使用集数\nEP01-02 EP05",
    episode=None, meta={"type": "scene"}))

cards.append(c("g-set-sc04", "setting", "scene04 矿洞", 6,
...
# ── design: 视觉参考（图片为主） ──
cards.append(c("g-des-r3", "design", "R3 苍霖 三视图", 1,
    "EP01-03 弱小版三视图\n可用作 @图片锚定",
    episode=None,
    images=[IMG["canglin_early"], IMG["canglin_weak"]],
    prompts=[f"角色设定图，正面全身定妆照，白色简洁背景。{CANG}。左脸颊一道从颧骨到下颌的竖向旧伤疤。眼神有韧劲但疲惫。{STYLE}。{POS}。{QS}"],
    meta={"type": "character_design", "r_id": "R3"}))

cards.append(c("g-des-r3-mid", "design", "R3 苍霖 中后期版", 2,
    "EP04-08 开胸狂暴版 / EP09-10 龙族废墟版\n供后期集数使用",
    episode=None,
    images=[IMG["canglin_mid"], IMG["canglin_v3"]],
    meta={"type": "character_design", "r_id": "R3"}))

cards.append(c("g-des-r1", "design", "R1 艾拉 参考图", 3,
    "星环法师战斗装全身参考",
    episode=None,
    images=[IMG["aila"]],
    prompts=[f"角色设定图，正面全身定妆照，白色简洁背景。{AILA}。表情冷淡锐利。{STYLE}。{POS}。{QS}"],
    meta={"type": "character_design", "r_id": "R1"}))

cards.append(c("g-des-r2", "design", "R2 巨龙 参考图", 4,
    "巨龙全身侧面参考",
    episode=None,
    images=[IMG["dragon_knight"]],
    prompts=[f"角色设定图，东方幻想巨龙全身侧面图，白色简洁背景。{DRAGON}。张开双翼展示翼展。{STYLE}。{POS}。{QS}"],
    meta={"type": "character_design", "r_id": "R2"}))

cards.append(c("g-des-miner", "design", "老矿工 参考图", 5,
    "路人角色参考",
    episode=None,
    images=[IMG["old_miner"]],
    meta={"type": "character_design"}))

cards.append(c("g-des-sc01", "design", "scene01 烬龙陆废巷 场景图", 6,
    "龙骨废墟全景 + EP01 苍霖初现概念图",
    episode=None,
    images=[IMG["scene_jinlong"], IMG["scene_ep01_v2"], IMG["scene_concept"]],
    prompts=[f"场景设定图，废墟巷道全景无人物。巨大龙骨化石如山矗立暗红色天空下，碎石墙壁两侧。暗红天光穿废墟缝隙形成长条体积光斑，微弱蓝色光粒子漂浮。{STYLE}。{POS}。{QS}"],
    meta={"type": "scene_design"}))

cards.append(c("g-des-sc04", "design", "scene04 矿洞 场景图", 7,
    "暂无实际参考图，待生成",
    episode=None,
    prompts=[f"场景设定图，地下矿洞内部无人物。狭窄岩壁，左侧墙挂暖橙色矿灯。洞穴深处巨大树根插入岩壁根表面脉动冷蓝色光。暖橙与冷蓝双色温交汇。{STYLE}。{POS}。{QS}"],
    meta={"type": "scene_design"}))

cards.append(c("g-des-props", "design", "关键道具图集", 8,
    "龙骨碎片：拇指大，表面血管纹路，蓝白光脉动如心跳，刻有符文\n"
    "苍霖掌心叶子：记忆物\n"
    "刻名龙鳞：龙的信物",
    episode=None,
    images=[IMG["prop_fragment"], IMG["prop_leaf"], IMG["prop_scale"]],
    meta={"type": "prop_design"}))

cards.append(c("g-des-lands", "design", "九大陆场景图", 9,
    "EP10剪影序列用\n01烬龙 02青霄 03星圣 04潮生 05巨神 06无昼 07冰封 08归墟 09神国",
    episode=None,
...
for sn, title, content in shot_specs:
    cards.append(c(f"e01-s{sn:02d}", "script", title, sn, content, episode=1, meta={"shot": sn}))

# ── setting: EP01 特有设定 ──
cards.append(c("e01-set-vfx", "setting", "龙骨碎片光效设定", 0,
    "## 四层拆分\n\n"
...
cards.append(c("e01-set-motion", "setting", "动作轨迹+姿态衔接", 1,
...
print(f"\nBy stage:")
for s in ["idea","script","setting","design","storyboard","img_prompt","vid_prompt"]:
